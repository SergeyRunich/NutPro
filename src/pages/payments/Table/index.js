...
import { Table, Popover, Button, Tag } from 'antd'

const TableOfPayments = ({ payments, status, type, isLoading }) => {
  ...
  const filteredTableData = useMemo(() => {
    if (status === 'All' && type === 'All') return transformTableData(payments)
    if (status === 'All' && type !== 'All')
      return transformTableData(payments).filter(item => item.gw === type)
    if (status !== 'All' && type === 'All')
      return transformTableData(payments).filter(item => item.status === status)
    if (status !== 'All' && type !== 'All')
      return transformTableData(payments).filter(item => item.status === status && item.gw === type)
  ...
  const columns = [
    ...
    {
      title: formatMessage({ id: 'Payments.Order' }),
      align: 'center',
      dataIndex: 'order',
      key: 'order',
      render: text => (
        <Button type="primary" size="default">
          <Link target="_blank" to={`/orders/${text}`}>
            {formatMessage({ id: 'Payments.openOrder' })}
          </Link>
        </Button>
      ),
    },
