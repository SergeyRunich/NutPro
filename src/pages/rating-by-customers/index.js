...
class RatingMenu extends React.Component {
  ...
  componentDidMount() {
    ...
    getRatingPeriod('customer', dates[0], dates[1]).then(async req => {
      const {
        intl: { formatMessage },
      } = this.props
      if (req.status === 200) {
  ...
  showDrawerDetails = record => {
    this.setState({
      macroVisible: true,
      selectedRatingDetails: record.dishes,
    })
  }
  ...
  render() {
    ...
    const columns = [
      {
        title: formatMessage({ id: 'Rating.Customer' }),
        dataIndex: 'name',
        key: 'name',
        // render: text => (
      ...
      },
      {
        ...
        render: text => (
          <span style={{ fontWeight: 'bold', fontSize: '16px' }}>
            {`${((text.count / stats.totalScores) * 100).toFixed(2)}%`}
          </span>
        ),
        // sorter: (a, b) => Number(a.scores.count / stats.totalScores) - Number(b.scores.count / stats.totalScores)
      },
    ]
    ...
    return (
      <Authorize
        ...
        to="/main"
      >
        <Helmet title="Ratings of dishes" />
                ...
                >
                  <div>
                    <Statistic
                      ...
                      title={formatMessage({ id: 'Rating.Total customers' })}
                  ...
                  <div>
                    <Statistic
                      style={{ textAlign: 'center' }}
                      title={formatMessage({ id: 'Rating.Total scores' })}
                  ...
                  <div>
                    <Statistic
                      style={{ textAlign: 'center' }}
                      title={formatMessage({ id: 'Rating.Scores per customer' })}
                      value={
                        stats.totalCustomers !== 0
                          ? (stats.totalScores / stats.totalCustomers).toFixed(2)
                          : 0
                      }
                    />
                  </div>
        ...
        <DishDetails
          ...
          onClose={this.onCloseDrawerDetails}
          data={this.state.selectedRatingDetails}
        />
        <IgnoredList
