...
class MenuLeft extends React.Component {
  ...
  generateMenuItems = () => {
    ...
    const generateItem = item => {
      const { key, title, url, icon, disabled, pro, roles, denied } = item
      if (roles && roles.length !== 0 && !roles.includes(role)) return
      if (denied && denied.length !== 0 && denied.includes(name)) return
      if (item.divider) {
    ...
    return menuData.map(menuItem => {
      if (menuItem.children) {
        if (menuItem.roles && menuItem.roles.length !== 0 && !menuItem.roles.includes(role)) return
        const subMenuTitle = (
          <span key={menuItem.key}>
            <span className={styles.title}>{menuItem.title}</span>
            {menuItem.icon && <span className={`${menuItem.icon} ${styles.icon}`} />}
    ...
    return (
      ...
      >
        <div className={styles.logo}>
          {menuSettings.collapsed && <div className={styles.logoText}>NP</div>}
          <div className={styles.logoContainer}>
            <img
              src={`resources/images/logo${menuSettings.collapsed ? '-mobile' : ''}.svg`}
              alt=""
            />
          </div>
        </div>
